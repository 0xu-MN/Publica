...
import { ProfileEditPage } from './ProfileEditPage'; // Import the new page!
...
export const PersonalDashboard: React.FC<PersonalDashboardProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'dashboard' | 'files' | 'posts' | 'messages' | 'scraps' | 'profile_edit'>(initialTab as any);
    ...
    // Tracks update to force refresh
    ...
    // Render Dedicated Profile Edit Page
    if (activeTab === 'profile_edit') {
        return (
            <ProfileEditPage
                onClose={() => setActiveTab('dashboard')}
                onSave={() => setProfileVersion(v => v + 1)}
            />
        );
    }
        ...
        <View className="flex-1 flex-row bg-[#050B14]">
            ...
            <View className="flex-1">
                {activeTab === 'dashboard' ? (
                            ...
                            <View className={isDesktop ? "flex-row gap-6 min-h-[600px]" : "gap-6"}>
                                ...
                                <View className={isDesktop ? "w-[340px]" : "h-[520px]"}>
                                    <View className="h-full">
                                        <ProfileCard
                                            key={profileVersion} // Force re-render on save
                                            readOnly={readOnly}
                                            targetUserId={targetUserId || undefined}
                                            // LINK TO DEDICATED PAGE
                                            onEditProfile={() => setActiveTab('profile_edit')}
                                            onChatPress={(userInfo) => {
                                                setActiveTab('messages');
                                                if (userInfo) setSelectedChatUser(userInfo);
                                                else setSelectedChatUser(targetUserId ? { id: targetUserId, name: 'User' } : undefined);
                                            }}
                                    ...
                                    <View className={isDesktop ? "flex-row gap-6 h-[280px]" : "gap-6"}>
                                        ...
                                        <View className={`bg-[#0F172A] rounded-3xl p-6 border border-white/5 flex-1 ${isDesktop ? '' : 'h-[300px]'}`}>
                                            <View className="flex-row items-center justify-between mb-4">
                                                <TouchableOpacity className="flex-row items-center gap-2" onPress={() => setCalendarVisible(true)}>
                                                ...
                                                <View className="flex-row items-center gap-2">
                                                    ...
                                                    <TouchableOpacity className="w-8 h-8 bg-blue-600 rounded-lg items-center justify-center" onPress={() => setCalendarVisible(true)}>
                            ...
                            <View className={`flex-1 p-6 ${selectedScrap && isDesktop ? 'max-w-[400px] border-r border-white/5' : ''}`}>
                                <View className="mb-6">
                                    <View className="flex-row items-center justify-between mb-4">
                                        ...
                                        <TouchableOpacity className="px-4 py-2 bg-slate-800 rounded-lg border border-white/10" onPress={loadScraps}>
                    ...
                    <View className="flex-1">
                        <View className="flex-1 bg-[#050B14] p-6 max-w-[1400px] w-full mx-auto">
                            {selectedProject ? (
                    ...
                    <View className="flex-1">
                        <View className="flex-1 bg-[#050B14] p-6 max-w-[1400px] w-full mx-auto">
                            <View className="bg-[#0F172A] rounded-3xl border border-white/10 p-6 h-full">
                                <ScrollView>
                                            ...
                                            .map(post => (
                                                <View key={post.id} className="mb-4 bg-slate-900/50 p-4 rounded-xl border border-white/5 flex-row items-center justify-between">
                                                    ...
                                                    <View className="flex-row gap-2 ml-4">
                                                        <TouchableOpacity className="bg-blue-600 px-3 py-1.5 rounded-lg"><Text className="text-white text-xs font-bold">수정</Text></TouchableOpacity>
                                                        <TouchableOpacity className="bg-red-500/20 px-3 py-1.5 rounded-lg border border-red-500/30" onPress={() => deletePost(post.id)}><Text className="text-red-400 text-xs font-bold">삭제</Text></TouchableOpacity>
                                                    </View>
                                                </View>
                                            ))
                                    ) : (
            ...
            <ProjectDetailModal visible={projectModalVisible} onClose={() => setProjectModalVisible(false)} projectName={selectedProject} />
...
const ProjectItem = ({ name, desc, progress, color, onPress, onWorkspace }: any) => (
    <View className="bg-slate-900/30 rounded-xl border border-white/5 overflow-hidden">
        ...
        <View className="px-4 pb-3 flex-row gap-2">
            <TouchableOpacity className="flex-1 bg-blue-600 py-2 rounded-lg flex-row items-center justify-center gap-2" onPress={onWorkspace}>
                <Icons.Sparkles size={12} color="#fff" />
                <Text className="text-white font-semibold text-xs">이어서 작업</Text>
            </TouchableOpacity>
            <TouchableOpacity className="flex-1 bg-white/5 py-2 rounded-lg flex-row items-center justify-center gap-2 border border-white/10" onPress={onPress}>
