...
import { TrendingUp, BarChart2, ArrowRight, Activity, Zap, Layers, FolderHeart } from 'lucide-react-native';
...
export const DashboardView: React.FC<DashboardViewProps> = ({ newsData, hotKeywords, user, onLoginPress }) => {
    ...
    return (
        <View className="flex-1 px-6 pb-6 pt-2">
            <View className="flex-1 max-w-[1400px] w-full mx-auto gap-4">

                {/* 1. Header Widget (Date & Auth) */}
                <View className="w-full h-[100px] rounded-[32px] overflow-hidden relative shadow-lg shadow-black/40 border border-white/10">
                    ...
                    <View className="flex-1 flex-row items-center justify-between px-10">
                        {/* Date Info */}
                        <View>
                            <Text className="text-slate-400 text-sm font-medium mb-1">오늘의 인사이트</Text>
                            <View className="flex-row items-baseline gap-2">
                                <Text className="text-white text-2xl font-bold">{formatDate(currentTime)}</Text>
                                <Text className="text-slate-400 text-base font-medium">{formatTime(currentTime)}</Text>
                            </View>
                        </View>

                        {/* Auth Info (Moved to Header) */}
                        <View className="flex-row items-center">
                            {user ? (
                                <View className="flex-row items-center gap-4">
                                    <View className="items-end">
                                        <Text className="text-slate-400 text-xs">Welcome back,</Text>
                                        <Text className="text-white text-base font-bold">{user.email?.split('@')[0]}님 반갑습니다 👋</Text>
                                    </View>
                                    <View className="bg-blue-600/20 px-3 py-1.5 rounded-full border border-blue-500/30">
                                        <Text className="text-blue-400 text-xs font-bold">Premium Plan</Text>
                                    </View>
                                </View>
                            ) : (
                                <TouchableOpacity onPress={onLoginPress} className="bg-blue-600 px-6 py-2.5 rounded-full hover:bg-blue-500 transition-colors">
                ...
                <View className="flex-1 flex-row gap-4 h-full min-h-[600px]">
                    {/* Left Column */}
                    ...
                    <View className="flex-[0.8] h-full min-h-[600px] gap-4">

                        {/* 1. Sidebar Hot Content (Keywords, Rising, Topics) */}
                        <View className="bg-slate-900/50 rounded-[32px] p-6 border border-white/5 gap-6 relative">
